  ...
  <div class="foundGroup">
      ...
      <div class="mainBox clearfix">
        ...
        <div class="mainL fl" @mouseup.stop="mainL($event)">
          ...
          <div class="btn" @click="finish($event)" :class="{'blue':isActive}">完成</div>
        </div>
    ...
    if(this.$route.query.type == 'edit'){
      ...
      this.$http.get('http://127.0.0.1:3000/jeic/api/teachingGroup/'+ this.$route.query.Id).then(function(data){
        ...
        for(var i=0;i<that.newBuild.length;i++){
          if(that.newBuild[i].studentList.length == 0  ||  JSON.stringify(that.newBuild[i].groupLeader) == '{}' || that.groupName == ''){
            that.isActive = false;
          }else{
            that.isActive = true;
          }
  ...
  methods: {
    ...
    deleteName(obj,i,index){
      if($(obj.target).parents('div').prev('h3').html() == "组长"){
        ...
        if(JSON.stringify(this.newBuild[i].groupLeader) == '{}'){
          this.isActive = false;
        }else{
          this.isActive = true;
        }
        ...
        if(this.newBuild[i].studentList == 0){
          this.isActive = false;
        }else{
          this.isActive = true;
        }
    ...
    rightStudent(obj,i){
      ...
      // 判断是否组长和组员都添加上了人员
      for(var i=0;i<this.newBuild.length;i++){
        if(this.newBuild[i].studentList.length == 0  ||  JSON.stringify(this.newBuild[i].groupLeader) == '{}' ){
          this.isActive = false;
        }else{
          this.isActive = true;
        }
