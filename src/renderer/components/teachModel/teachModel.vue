    ...
    <div class="subject" v-show="!allClassMask && !chooseGroupMask  && !lookGroupMask">
      ...
      <div class="conent">
        ...
        <div class="allClass" v-show="$route.query.classType == 'all'">
          ...
          <p>全班教学模式为班级整体教学</p>
          <!-- 按钮  -->
          <div class="btn" style="margin: 2rem auto" @click="allConfirm()">确认选择</div>
        </div>
        ...
        <div class="groupClass"  v-show="$route.query.classType == 'group'">
          ...
          <div v-show="groupTeachData.length != 0">
            <div class="groupList">
              <ul>
            ...
            <div class="clearfix buttonBox">
              <div class="smallAdd fl" @click="addGroup()"><i class="iconfont icon-jiahao"></i></div>
              <div class="btn fl" @click="groupConfirm()">确认选择</div>
            </div>
          </div>
        </div>
      </div>
        ...
        <div class="branchList">
          <ul>
            <li class="line" v-for="(item,index) in lookGroupData" :key='index'>
                ...
                <div class="clearfix" >
                  <div class="fl lineLeft" v-if=" JSON.stringify(item.groupLeader) != '{}'">
  ...
  methods: {
    ...
    tabTeachModel(i){
      if(i==1){
        ...
        this.groupIndex = 2;
        this.$http.get('http://127.0.0.1:3000/jeic/api/teachingGroup/syncStudent?classId=' +  this.$store.state.state.classId + '&subjectId=' + this.$store.state.state.subjectId).then(function(data){
        });
        this.$router.push({
          path:'/teachModel',
          query:{
    ...
    allConfirm() {
      ...
      this.$store.dispatch('getChangUser', 'all');
	  this.$socket.emit("group", {"name": "complateGroupList","AnswerType":"1","groupId":""});
    ...
    groupConfirm(){
      if(this.tabIndex != -1){
        ...
        this.$store.dispatch('getChangUser', 'group');
		...
		this.$socket.emit("group", {"name": "complateGroupList","AnswerType":"2","groupId":this.groupTeachData[this.tabIndex].id});
      }else{
        this.allClassMask = true;
        this.msg = '请先选择分组';
        setTimeout(() => {
    ...
    lookGroup(i){
      ...
      /* this.$http.get('http://127.0.0.1:3000/jeic/api/teachingGroup/15ad91e0-e0e5-11e9-b98f-612362a7258c').then(function(data){
        that.lookGroupData = data.data.data.userGrouplist;
      }); */
    },
    // 分组教学 --- 关闭 和 返回 查看分组弹层
...
<style>
.subject{width: 80rem; height:47rem; border-radius: 6rem; position: fixed; top: 0; left: 0; bottom: 5.25rem; right: 0; margin: auto;color: #333333;}
...
.subject .conent{width: 100%; height: 70%; text-align: center; margin: 0 auto; background: #898989; padding-top: 2rem; box-sizing: border-box;border-bottom-left-radius: 6px;border-bottom-right-radius: 6px;}
.subject .allClass{padding: 3rem 0; box-sizing: border-box;}
.subject .allClass img{width: 13rem; height: 13.875rem;}
.subject .allClass p{font-size: 1.45rem; margin-top: 1.25rem; color: #fff;}
.subject .btn{width: 20rem; height: 4rem; line-height: 4rem; text-align: center; background: #4092f4; color: #fff; border-radius: 40px; margin: 2rem 0 0 25rem; cursor: pointer; font-size: 1.6rem;}

.subject .groupClass .addGroup{width: 18.75rem; height: 15rem; font-size: 1.6rem; background: #fff; border-radius: 5px; margin: 4rem auto 0; box-shadow: 10px 10px 1rem #46494a; box-sizing: border-box; padding-top: 4rem;}
.subject .groupClass i{cursor: pointer; font-size: 70px; color: #4092f4;}
.subject .groupClass .buttonBox{width: 89%;margin: auto;position: relative;}
.subject .groupClass .smallAdd{width: 5rem; height: 5rem; position:absolute;bottom: 0; border-radius: 5px; background: #fff; box-sizing: border-box; text-align: center; padding-top: 1.6rem;margin-left: 2rem;cursor: pointer; box-shadow: 0 0 20px #46494a;}
.subject .groupClass .smallAdd i{font-size: 3rem;}
...
.subject .groupClass .groupList{height: 24rem;overflow-y: auto;}
.subject .groupClass .groupList li{width: 100%; height: 6.5rem; box-shadow: 0 0 1rem #46494a; background: #fff; border-radius: .5rem; margin-top: 1.2rem;}
