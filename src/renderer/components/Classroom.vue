		...
		<div class="classroom">
			<div class="classroom_begin" v-if="!classroomState">
				...
				<div class="banjinianji clearfix">
					<div>
						班级:
						<select id="banjiid" :value="checkdedClassroom" v-model="checkdedClassroom" @change="selectClass(checkdedClassroom)">
							<option value="请选择班级">请选择班级</option>
							<option v-for="item in classroomList" :value="item.classId" :key="item.classId">{{item.gradeName+item.className+'班'}}</option>
						</select>
					</div>
					<div>
						科目:
						<select id="xuekeid" :value="subject"  @change="selectSubject()">
			...
			<div class="classroom_begin" v-if="classroomState">
				<div class="box_title">
					<span class="close_box fr" @click="cancel()">
						<img src="../assets/img/close.svg" alt />
					</span>
				...
				<div>
					...
					<span class="btn cancelcolor" @click="cancel()">取消</span>
				</div>
			</div>
		</div>
	...
	export default {
		...
		sockets: {
			...
			selectClass(data) {
				...
				this.selectClass(this.checkdedClassroom);
			},
			selectSubject(data) {
				this.subject= data[0];
		...
		methods: {
			...
			selectClass(classId) {		
				if (classId != "请选择班级") {
					//选择班级
					this.$http.get(this.configure.jeucIp + "/ea/eaUserCourse/subject?uid=" + this.userId + "&cid=" + classId).then(
						jdata => {
							if (jdata.data.ret == 200) {
			...
			selectSubject(){
				var select=document.getElementById("xuekeid");
				var index=select.selectedIndex-1;
					this.checkdedSubject=this.subjectList[index];
			...
			beginClass(classId, subjectId, subjectName) {
				...
				if (classId == "请选择班级") {
					...
					this.$http.post("http://127.0.0.1:3000/jeic/api/classRecord/startClass", {
			...
			endClass() {
				this.$socket.emit("group", {"name": "xiakepad"});
				if (this.lupingState) {
