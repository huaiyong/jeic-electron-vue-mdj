	...
	<div class="zmj_resourceList">
		...
		<div class="zmj_resourceContent clearfix">
			<div class="zmj_resourceSidebar">
				<ul>
					<router-link tag="li" :to="{path:'',query:{testType:'resource'}}" exact :class="{'active':sideBarIndex==0}" @click.native="sideBarTab(0)"><i class="iconfont icon-yun1"></i>云资源</router-link>
					<router-link tag="li" :to="{path:'',query:{testType:'test'}}" exact :class="{'active':sideBarIndex==1}" @click.native="sideBarTab(1)"><i class="iconfont icon-yun1"></i>云试题</router-link>
				</ul>
			</div>
	...
	export default {
		...
		sockets: {
			...
			yunziyuanScroll(data) {
				var conheight = $(".zmj_resourceType ul").height()- $(".zmj_resourceType").height();
				$(".zmj_resourceType").scrollTop(data * conheight);
			},
			yunshitiScroll(data) {
				var conheight = $(".zmj_resourceTest ul").height()-$(".zmj_resourceTest").height();
				$(".zmj_resourceTest").scrollTop(data * conheight);
			}
		},
		methods: {
			...
			getQuestionList(){  //云试题
				this.$http.get(this.configure.resourceIp + "/sync?userId=" + this.userId + "&type=homework").then((res)=>{
			...
			getResourceList(){ //云资源
				this.$http.get(this.configure.resourceIp + "/sync?userId=" + this.userId + "&type=res").then((res)=>{
