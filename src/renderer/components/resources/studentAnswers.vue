	...
	<div class="zmj_answer">
		...
		<div class="zmj_answerStudent">
			...
			<div class="zmj_answerStudentList">
				...
				<div v-if="!pattern" class="zmj_groupStudent">
					<div v-for="i in groupStudent" :key="i.id" class="list">
						<p v-text="i.name"></p>
						<div class="clearfix">
		...
		<div class="zmj_xaifaBox" v-if="studentResultState">
			<div class="zmj_studentResult">
				<div class="resultTitle clearfix"><span><i v-text="groupName" v-if="groupName.length>0&&pattern==0"></i> <i v-text="studentName"
		...
		<div class="loading" v-show="loadingShow==true || loadingShowState==true">
			<i class="iconfont icon-guanbi5" @click="back()"></i>
			<img src="../../assets/img/shiti.gif" alt="">
		</div>
	</div>
</template>
...
<script>
	...
	export default {
		...
		sockets: {
			...
			lowerhairHeadScroll(data){
				var conheight = $(".zmj_xaifaList .groupLeader").height();
				$(".zmj_xaifaList").scrollTop(data* conheight);
			},
			lowerhairTeamScroll(data){
				var conheight = $(".zmj_xaifaList .zmj_teamMember").height();
				$(".zmj_xaifaList").scrollTop(data* conheight);
			}
		...
		methods: {
		    ...
		    yunshitiMove(direction){  //云试题左侧滚动条
				...
				if (direction == "top" && this.yuntishiScroll<maxHeight) {
					this.yuntishiScroll += 40;
			...
			checkedStudentFun(i, index, check) {
                ...
                $(".zmj_allGroup").eq(i).find("li").eq(index + 1).find("input").prop("checked",check);
				...
				if (this.pattern) {
					...
					this.$http.post("http://127.0.0.1:3000/jeic/api/sendRecord", {
						...
						if (res.data.ret == 200) {
							...
							that.$socket.emit('xiafaOk', {
								'name': "recordId",
								'data': res.data.data
							});
							that.checkedState = true;
							that.lowerHairState=false;
							that.$store.dispatch("getDatiqi","请先结束答题");
					...
					this.timer = setInterval(function() { //查询有谁作答
						that.$http.get("http://127.0.0.1:3000/jeic/api/sendRecord/getAnsweredList").then(function(res) {
							if (res.data.ret == 200) {
						        ...
						        that.arr=[];
								var tem = that.remember.concat([]);
								var flag = 0;
								tem.forEach(function(item,index){
									if(that.signRemember.indexOf(item.device)!=-1 || that.signRemember.indexOf(item.id)!=-1){
										that.remember.splice((index-flag),1);
		                                that.arr.push(item);
										flag ++;
									};
								});
                               
								that.arr.forEach(function(item){
									that.remember.push(item);
								});
...
<style>
	...
	.zmj_groupStudent .list {
		font-size: 1.6rem;
		color: #fff;
		padding: 1.6rem;
		border-bottom: .1rem solid  #ccc;
	}
   .zmj_groupStudent .list:last-child{
	   border: none;
   }
	.zmj_groupStudent ol {
		width: 22%;
