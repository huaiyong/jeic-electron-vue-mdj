<template>
	<div class="wrapper">
		<div class="getClasstjtc">
			<div class="tcContent">
				<!-- 分组选择左侧栏 -->
				<div class="fzleft" v-if="answerType==3">
					<p>
				...
				<div class="menberight">
					<!-- 班级答题统计 -->
					<div class="pingjuntrue" v-if="answerType=='1'">
					...
					<div class="pingjuntrue" v-if="answerType=='2'">
						<ul class="banjiuldati">
							<li>测试平均正确率：
								<p class="pingjunzitiset">{{averagelv2}}%</p>
							</li>
					...
					<!-- 班级答题统计 全组模式 点击全部 -->
					<div class="pingjuntrue" v-if="answerType=='3'&&allGroupChecked">
						<ul class="banjiuldati">
							<li>测试平均正确率：
								<p class="pingjunzitiset">{{averagelv2}}%</p>
							</li>
					...
					<div class="grop_list_show classtableset">
						...
						<div style="text-align:center;margin-top: 2rem;">
							<router-link tag="span" v-if="!$route.query.changeTab" id="switchPai" class="lvsebtn" @click.native="switchTab('showTable')"  :to="{path:'',query:{tab:'column',changeTab:true}}" exact >查看表格</router-link>
							<router-link tag="span" v-if="$route.query.changeTab" id="switchPai" class="lvsebtn" @click.native="switchTab('showPic')"  :to="{path:'',query:{tab:'table',changeTab:false}}" exact >查看图表</router-link>
	...
	export default {
		...
		methods: {
			...
			addClass:function(index,id){ //点击组
				  ...
				  var that=this;
				  this.$http.get("http://localhost:3000/jeic/api/answerResult/getAnswerByRecordId?recordId="+this.eachRecordId+'&type=1'+'&teachinggroupId='+this.teachingGroupId+'&usergroupId='+id).then(function(res) {
				  	var tishuliangnum=[];
				  	var banjibaifenbi=[];
				  	var averagelv=0
				  ...
				  this.$http.get("http://127.0.0.1:3000/jeic/api/answerResult/getDataGroupByUser?recordId="+this.eachRecordId+'&type='+this.model+'&usergroupId='+id+'&teachinggroupId='+this.teachingGroupId).then(function(res) {
			...
			allClass:function(id){ //点击全部
				...
				sessionStorage.removeItem("groupId");
				var that=this;
				this.$http.get("http://localhost:3000/jeic/api/answerResult/getAnswerByRecordId?recordId="+this.eachRecordId+'&type=1'+'&teachinggroupId='+this.teachingGroupId).then(function(res) {
					var tishuliangnum=[];
					var banjibaifenbi=[];
					var averagelv=0
					var chushu = res.data.data.length;
				...
				// 获取已答题和未答题人数
				this.$http.get("http://127.0.0.1:3000/jeic/api/answerResult/getDataGroupByUser?recordId="+this.eachRecordId+'&type='+this.model+'&teachinggroupId='+this.teachingGroupId).then(function(res) {
					that.haveTheAnswerCount = res.data.data.haveTheAnswerCount
					that.notTheAnswerCount = res.data.data.notTheAnswerCount
				});
		...
		created() {
			...
			if(sessionStorage.getItem("groupId")){
				var that = this;
				this.$http.get("http://localhost:3000/jeic/api/answerResult/getAnswerByRecordId?recordId=" + this.eachRecordId +
					'&type=1' + '&teachinggroupId=' + this.teachingGroupId + '&usergroupId=' + sessionStorage.getItem("groupId")).then(function(res) {
					var tishuliangnum = [];
					var banjibaifenbi = [];
					var averagelv = 0
					var chushu = res.data.data.length;
				...
				this.$http.get("http://127.0.0.1:3000/jeic/api/answerResult/getDataGroupByUser?recordId=" + this.eachRecordId +
					'&type=' + this.answerType + '&usergroupId=' + sessionStorage.getItem("groupId") + '&teachinggroupId=' + this.teachingGroupId).then(function(res) {
					that.haveTheAnswerCount = res.data.data.haveTheAnswerCount
					that.notTheAnswerCount = res.data.data.notTheAnswerCount
				});
				...
				this.$http.get("http://localhost:3000/jeic/api/answerResult/getAnswerByRecordId?recordId=" + this.eachRecordId +
					'&type=1' + '&teachinggroupId=' + this.teachingGroupId).then(function(res) {
					var tishuliangnum = [];
					var banjibaifenbi = [];
					var averagelv = 0
					var chushu = res.data.data.length;
				...
				// 获取已答题和未答题人数
				this.$http.get("http://127.0.0.1:3000/jeic/api/answerResult/getDataGroupByUser?recordId=" + this.eachRecordId + '&type=' +
					this.answerType + '&teachinggroupId=' + this.teachingGroupId).then(function(res) {
					that.haveTheAnswerCount = res.data.data.haveTheAnswerCount
					that.notTheAnswerCount = res.data.data.notTheAnswerCount
				});
			}
