				...
				<table>
					...
					<tbody>
						<tr v-for="name in names">
							<td class="wx_relative"><img class="redFlower" v-if="redfloewer==1" src="../../assets/img/icon_xiaohonghua.png" alt="">{{name.userGroupName}}</td>
							<td>{{name.userName}}</td>
	...
	export default ({
		...
		methods: {
			...
			getParams() {
				...
				this.redfloewer = this.$route.params.red; // 取到路由带过来的参数
				console.log(this.eachRecordId)
				...
				this.$http.get("http://127.0.0.1:3000/jeic/api/answerResult/getDataGroupByUser?recordId=" + this.eachRecordId +
					'&type=' + this.answerType + "&teachinggroupId=" + this.teachingGroupId+'&usergroupId='+routerParams).then(function(res) {
