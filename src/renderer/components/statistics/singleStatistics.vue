<template>
	<div class="wx_wrapper">
		<div class="wx_singleTest">
			<!-- 柱状图 -->
			<div class="wx_zhutuabcd">
				<div id="myChart" style="width:100%;height:90%;"></div>
				<!--   选项人数  -->
			</div>

			<!-- 试题讲解 -->
			<div class="wx_studentDatirenshu">
				<div class="tihaotcset">
					<div class="wx_tiganul">
						<div>
                ...
                <div class="wx_selectkuang" v-show="clickEcharts">
                	<div v-if="model==1" class="wx_stuList">
                		<p class="wx_slectStrcolor"><span class="wx_xuanxiangkuang" v-html="xuanxiang"></span>选项：</p>
                		<ul class="wx_xuanxiangullist">
                			<li v-for="stulist in stulists">{{stulist.realname}}</li>
                		</ul>
                	</div>
                	<div v-if="model==3" class="wx_stuList">
                		<p class="wx_slectStrcolor"><span class="wx_xuanxiangkuang" v-html="xuanxiang"></span>选项：</p>
                		<ul class="wx_xuanxiangullist">
                	...
                	<div v-if="model==2" class="wx_stuList">
                		<p class="wx_slectStrcolor"><span class="wx_xuanxiangkuang" v-html="xuanxiang"></span>选项：</p>
                		<ul class="wx_xuanxiangullist">
                			<li v-for="stulist in stulists">
...
<script>
	...
	export default {
		...
		mounted() {
			this.tihao = this.$route.params.id;
			this.getParams();
			var that = this;
			$(".wx_tiganul").on("click", "img", function() {
		...
		sockets: {
			...
			dantishitijiangjieScroll(data) {
				...
				$(".wx_studentDatirenshu").scrollTop(data * conheight);
			}
		...
		methods: {
			...
			getResult() {
				...
				var ydata = [];
				// 获取单道试题的答题结果
				this.$http.get("http://127.0.0.1:3000/jeic/api/answerResult/getDataByQuestionId?recordId=" + this.recordId +
					"&type=" + this.model + "&datamark=" +
