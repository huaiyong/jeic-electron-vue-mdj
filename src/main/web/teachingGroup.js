 ...
 * 同步学生
 */
router.get('/syncStudent', function(req, res, next) {
	var classId = req.param('classId');
	var subjectId = req.param('subjectId');
	var teachingGroupParam = new TeachingGroup();
	teachingGroupParam.classId = classId;
	teachingGroupParam.subjectId = subjectId;
	DB.findTeachingGroup(teachingGroupParam,async function (rows) {
		if(rows.length > 0){
			for(var teachingGroup of rows){
				var userGroupParam = new UserGroup();
				userGroupParam.teachinggroupId = teachingGroup.id;
				var rows1 = await DB.findUserGroup1(userGroupParam);
				if(rows1.length > 0){
					//查询班级全部学生
					var studentGroup = [];
					var studentRows = await DB.findStudentList(classId);
					if(studentRows.length > 0){
						//var j=0;j < rows1.length;j++
						for(var j=0;j < rows1.length;j++){
							//console.log("1111111111"+isObj(studentRows,rows1[j].student_id));
							if(isObj(studentRows,rows1[j].student_id)==undefined){
								//console.log("未包含的id"+rows1[j].id);
								//该学生不在学生student表里面，把他删除.
								DB.deleteStudentUserGroupById(rows1[j].id);
							};
						}
					}
				}	
