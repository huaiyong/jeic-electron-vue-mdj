...
const Constants = require('../src/main/common/Constants');
let iModeType;			// 答题器状态
...
function keyCallBack(BaseID,KeyID,KeySN,iMode,Time,sInfo) {
    ...
    if(iModeType==1){
        Constants.userMap[KeySN] = KeySN;
    }else if(iModeType==14){	//答题状态下记录答题的设备
        Constants.answer[KeySN] = KeySN;
        //存储答题信息
        if(sInfo.indexOf(":") != -1) {
	        ...
	        Constants.answerList.push(answerResult);
        }
    }else if(iModeType==13){
    	if(Constants.winner.length==0){
    		Constants.winner.push(KeySN);
    	}
...
exports.getSignin = function() {
    const userList = [];
    for(var key in Constants.userMap){
        userList.push(key);
    }
    return userList;
}
...
exports.answerStart = function(voteType,answerStarParam) {
    Constants.answer = {};		//清空答题记录
    Constants.answerList = [];
    Constants.padAnswerList = [];		// 添加到全局变量缓存中  结束下发使用
	Constants.padAnswerUserList = [];	
...
exports.getAnswer = function() {
    let userAnswerList = [];
    for(var key in Constants.answer){
